...
const sidebarLinks = [
  ...
  { name: 'Staff & Roles', href: '/admin/staff', icon: Shield, adminOnly: true },
  { name: 'B2B Requests', href: '/admin/b2b-requests', icon: Briefcase, adminOnly: true },
  { name: 'Enquiries', href: '/admin/enquiries', icon: MessageSquare, adminOnly: true },
  { name: 'Reports', href: '/admin/reports', icon: BarChart2, adminOnly: true },
  { name: 'Shipping', href: '/admin/shipping', icon: Truck, adminOnly: true },
...
export default function AdminSidebar({ initials, userName, role, onClose }: { initials: string; userName: string; role?: string; onClose?: () => void }) {
  ...
  return (
    <aside className="w-full lg:w-64 bg-[#21050A] text-white flex flex-col h-full flex-shrink-0 relative z-20">
      ...
      <nav className="flex-1 overflow-y-auto px-4 space-y-1.5 custom-scrollbar">
        {sidebarLinks.filter(link => role === 'SUPER_ADMIN' || !link.adminOnly).map((link) => {
      ...
      <div className="p-4 mt-auto border-t border-white/10 bg-[#1A0307]">
        <div className="flex items-center gap-3 px-2 py-2 rounded-xl cursor-pointer hover:bg-white/5 transition-colors">
          ...
          <div className="flex-1 overflow-hidden">
            ...
            <p className="text-xs text-gray-400">{role === 'SUPER_ADMIN' ? 'Super Admin' : 'Manager'}</p>
          </div>
