...
import { 
  Store, CreditCard, Bell, Shield, MapPin, Save, Lock, Mail, Phone, ToggleLeft, ToggleRight, Map, Check, Gift
} from 'lucide-react';
import Link from 'next/link';
 
export default function AdminSettingsPage() {
  const [activeTab, setActiveTab] = useState<'general' | 'locations' | 'payments' | 'notifications' | 'security' | 'banner'>('general');
  ...
  const [bannerTitle, setBannerTitle] = useState("Festive Special");
  const [bannerSubtitle, setBannerSubtitle] = useState("15% OFF");
  const [bannerText, setBannerText] = useState("Celebrate the season with our premium handcrafted cakes and desserts. Limited time offer!");
  const [bannerImageUrl, setBannerImageUrl] = useState("https://images.unsplash.com/photo-1578985545062-69928b1d9587?q=80&w=1089&auto=format&fit=crop");
  const [bannerLinkUrl, setBannerLinkUrl] = useState("/shop?category=cakes");
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setBannerLinkUrl(data.bannerLinkUrl);
        }
      } catch (err) {
        console.error(err);
      } finally {
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-64 flex-shrink-0">
          <nav className="flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-visible pb-4 lg:pb-0 scrollbar-thin">
            ...
            <button onClick={() => setActiveTab('banner')} className={getTabClass('banner')}>
              <Gift size={18} /> Festive Banner
            </button>
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          {activeTab === 'banner' && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div>
                  <h2 className="text-lg font-bold text-gray-900">Festive Offer Banner</h2>
                  <p className="text-sm text-gray-500">Configure the popup modal and floating gift widget for the storefront.</p>
                </div>
                <button 
                ...
                >
                  {bannerActive ? (
                    <ToggleRight size={44} className="text-[#C89F5F]" />
                  ) : (
                    <ToggleLeft size={44} className="text-gray-300" />
                  )}
                </button>
              ...
              <div className="p-6 flex flex-col gap-6">
                <p className="text-sm text-gray-600 bg-amber-50 border border-amber-100 rounded-lg p-4">
                  The Festive Banner is currently <strong>{bannerActive ? "Active" : "Inactive"}</strong>. {bannerActive ? "Customers will see the popup modal when they visit the store." : "It is completely hidden from the storefront."}
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Banner Subtitle / Offer</label>
                    <input 
                      type="text" 
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Banner Text Description</label>
                    <textarea 
                      rows={3} 
                      value={bannerText} 
