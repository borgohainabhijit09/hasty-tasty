...
export default function B2BPage() {
  ...
  const getB2BPrice = (product: any, qty: number) => {
    ...
    return product.b2bPrice || product.basePrice || product.price;
  };

  const enquirySubtotal = enquiryCart.reduce((total, item) => total + getB2BPrice(item.product, item.qty) * item.qty, 0);
  const retailSubtotal = enquiryCart.reduce((total, item) => total + (item.product.basePrice || item.product.price) * item.qty, 0);
                      ...
                      <div className={`p-3 ${viewMode === 'list' ? 'flex-1 flex flex-row items-center justify-between' : 'flex flex-col flex-1'}`}>
                        ...
                        <div className={`flex items-end justify-between mb-3 ${viewMode === 'list' ? 'flex-col items-end gap-1 mb-0 mx-4' : ''}`}>
                          <div>
                            ...
                            <p className="text-[13px] font-semibold text-gray-500 line-through">₹{p.basePrice || p.price}</p>
                          </div>
                          <div className={`text-right ${viewMode === 'list' ? 'text-right' : ''}`}>
                            ...
                            <p className="text-base font-bold text-[#2E7D32]">
                              {p.pricingTiers && p.pricingTiers.length > 0 ? (
                                (() => {
                                  const sorted = [...p.pricingTiers].sort((a,b) => a.price - b.price);
                                  return <><span className="text-xs font-normal text-gray-500">From </span>₹{sorted[0].price}</>;
                                })()
                              ) : (
                                "₹" + (p.b2bPrice || p.basePrice || p.price)
                              )}
                            </p>
                          </div>
                        </div>
                  ...
                  {enquiryCart.length === 0 ? (
                    ...
                    enquiryCart.map((item) => (
                      <div key={item.product.id} className="flex justify-between items-start border-b border-gray-200/60 pb-4">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-[13px] font-bold text-[#3A1E14]">₹{getB2BPrice(item.product, item.qty) * item.qty}</span>
                          <button 
