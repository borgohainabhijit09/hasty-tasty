...
function ShopContent() {
  ...
  useEffect(() => {
    ...
    const newCategories = categoryParam ? categoryParam.split(",") : [];
    setSelectedCategories(prev => {
      const isSame = prev.length === newCategories.length && prev.every(c => newCategories.includes(c));
      return isSame ? prev : newCategories;
    });

    const searchParam = searchParams.get("search") || searchParams.get("product") || "";
  ...
  useEffect(() => {
    ...
    const fetchWithRetry = async (url: string, retries = 3, delayMs = 800): Promise<Response | null> => {
    ...
    const fetchCategories = async () => {
      const res = await fetchWithRetry(`${apiUrl}/api/categories?b2c=true`);
      if (res) {
        try {
          const data = await res.json();
          setCategories(data);
                  ...
                  >
                    ...
                    <div className={`p-4 flex flex-col flex-1 h-full ${viewMode === 'list' ? 'justify-center p-6' : ''}`}>
                      ...
                      <Link href={`/product/${p.slug}`} className="block">
                        <h4 className="text-[14px] font-bold text-[#3A1E14] mb-1.5 line-clamp-1 hover:underline">{p.name}</h4>
                      </Link>
