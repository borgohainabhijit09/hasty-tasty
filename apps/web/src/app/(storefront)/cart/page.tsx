...
export default function CartPage() {
  ...
  const subtotal = items.reduce((total, item) => total + calculateItemPrice(item, isB2B) * item.quantity, 0);
            ...
            {items.map((item) => (
              <div key={item.id} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center pb-4 border-b border-gray-50 last:border-0 last:pb-0">
                ...
                <div className="col-span-1 md:col-span-2 flex justify-between md:justify-center items-center">
                  ...
                  <span className="font-medium text-gray-600 text-sm">₹{calculateItemPrice(item, isB2B)}</span>
                </div>
                ...
                <div className="col-span-1 md:col-span-2 flex justify-between md:justify-center items-center">
                  ...
                  <div className="flex items-center gap-2 border border-gray-200 rounded-md px-2 py-1 bg-white">
                    ...
                    <input 
                      ...
                      className="text-xs font-bold w-8 text-center bg-transparent border-none outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                    <button 
                ...
                <div className="col-span-1 md:col-span-2 flex justify-between md:justify-end items-center">
                  ...
                  <span className="font-bold text-[#2E7D32] text-sm md:text-base">₹{calculateItemPrice(item, isB2B) * item.quantity}</span>
                </div>
              </div>
            ))}
          </div>
