...
import { useCartStore, calculateItemPrice } from "@/store/useCartStore";
...
import { getAddresses } from "@/app/actions/store";
import { getShippingRates } from "@/app/admin/(dashboard)/shipping/actions";

export default function CheckoutPage() {
  const { items, clearCart } = useCartStore();
  const router = useRouter();
  const { data: session, status } = useSession();
  const isB2B = (session?.user as any)?.role === 'B2B_CUSTOMER';
  ...
  const adjustedItems = useMemo(() => {
    if (locationRate <= 0 || items.length === 0) {
      return items.map(item => ({
        ...item,
        price: calculateItemPrice(item, isB2B)
      }));
    }
    const totalQty = items.reduce((sum, item) => sum + item.quantity, 0);
    if (totalQty === 0) return items.map(item => ({ ...item, price: calculateItemPrice(item, isB2B) }));
    
    const markupPerUnit = locationRate / totalQty;
    return items.map(item => {
      const basePrice = calculateItemPrice(item, isB2B);
      return {
        ...item,
        price: Math.round((basePrice + markupPerUnit) * 100) / 100
      };
    });
  }, [items, locationRate, isB2B]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      // Fetch Addresses
      const { data: addrs } = await getAddresses();
      ...
      const { data: rates } = await getShippingRates();
      if (rates) {
                ...
                <div className="space-y-4">
                  {addresses.length === 0 ? (
                    ...
                    <>
                      ...
                      {!isB2B && selectedAddress && (
                        <div className="mt-6 p-4 bg-[#FAF8F5] rounded-xl border border-[#EAE2DB]">
                          <div className="flex items-start gap-3">
                            <MapPin className="text-[#C89F5F] shrink-0 mt-0.5" size={20} />
                            <div className="flex-1">
                              <h4 className="font-bold text-[#3A1E14] text-sm mb-1">Delivery Location</h4>
                              <p className="text-xs text-gray-600 mb-3">
                                Please share your exact location so our delivery partner can find you easily.
                              </p>
                              
                              {location ? (
                                <div className="flex items-center gap-2 text-sm text-green-700 font-medium bg-green-50 px-3 py-2 rounded-lg border border-green-200">
                                ...
                                <div>
                                  <button 
                                    ...
                                    className="bg-white border border-[#C89F5F] text-[#5c2a1c] px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-[#C89F5F] hover:text-white transition-colors disabled:opacity-50"
                                  >
                                    {isCapturingLocation ? <Loader2 size={16} className="animate-spin" /> : <MapPin size={16} />}
