...
export default function CartDrawer() {
  ...
  const totalAmount = items.reduce((total, item) => total + calculateItemPrice(item, isB2B) * item.quantity, 0);
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <div className="flex items-center gap-3 border border-gray-200 rounded-full px-2 py-1 bg-white">
                          ...
                          <input 
                            ...
                            className="text-xs font-medium w-8 text-center bg-transparent border-none outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                          />
                          <button 
                        ...
                        <span className="font-bold text-[#2E7D32] text-sm">₹{calculateItemPrice(item, isB2B) * item.quantity}</span>
                      </div>
                    </div>
                  </div>
                ))
