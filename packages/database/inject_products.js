...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || "postgresql://postgres.xdxadyrdkppxxvizzloq:Advikrini%401408@aws-1-ap-southeast-2.pooler.supabase.com:6543/postgres",
...
async function inject() {
  try {
    console.log("Seeding categories first...");
    for (const [name, id] of Object.entries(categoriesMap)) {
      const slug = generateSlug(name);
      await pool.query(`
        INSERT INTO "Category" (id, name, slug, "isB2C", "isB2B", "updatedAt")
